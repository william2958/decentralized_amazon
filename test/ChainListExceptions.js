...
contract('ChainList', function(accounts) {
    ...
    // Test case: buying an article when no article is for sale yet
    it("should throw an exception if you try to buy an article when there is no article for sale", function() {
        ...
        return ChainList.deployed().then(function(instance) {
            ...
            // this says that unlike usually where we provide a callback function here we expect it to fail and go to the
            // catch block instead
            .then(assert.fail)
            .catch(function(error) {
                assert(error.message.indexOf('invalid opcode') >= 0, "error should be invalid opcode")
            }).then(function() {
                return chainListInstance.getNumberOfArticles.call();
            }).then(function(data) {
